...
void pop (StackEntry *pe,Stack *ps)
{
	*pe=ps->Entry[--ps->top];
	...
	{
		if(ps->Entry[i]< ps->min){ps-> min=ps->Entry[i];}
	}
}
...
// ps -> StackEntry
// pre: stack is initialized and not empty and poped number not equal min
// post : return minimum number in the stack
void StackMin(StackEntry *pe ,Stack *ps)
{
	*pe=ps->min;
}
#else
